...
GraphicEngine::GraphicEngine(void* nativeApp)
{
    ...
    assert(nativeApp != nullptr);
    android_app* app = (android_app*)nativeApp;
    surface = eglCreateWindowSurface(display, config, static_cast<ANativeWindow*>(app->window), nullptr);
    ...
    m_deviceContext->m_nativeApp = app;
}
