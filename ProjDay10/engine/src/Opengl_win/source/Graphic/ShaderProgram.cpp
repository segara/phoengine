...
std::string RemoveExtension(const std::string& filename, const std::string& extension) {
    if (filename.size() >= extension.size() &&
        ...
        return filename.substr(0, filename.size() - extension.size()); // 확장자 제거
    }
    return filename; // 확장자가 없으면 그대로 반환
}
bool ShaderProgram::load(const char* shaderfile)
{
    ...
    WriteToFile(RemoveExtension(shaderfile, ".fx") + "_vs.glsl", result_vs.sourceCode);
    ...
    WriteToFile(RemoveExtension(shaderfile, ".fx") + "_ps.glsl", result_ps.sourceCode);
    //for gles end
